import { ComposableMap, Geographies, Geography } from "react-simple-maps";
// Import topojson directly — no network fetch, no 404 risk
// eslint-disable-next-line @typescript-eslint/no-explicit-any
import topoData from "world-atlas/countries-110m.json";

// world-atlas uses ISO 3166-1 numeric IDs. Maps alpha-2 → numeric.
const ALPHA2_TO_NUMERIC: Record<string, string> = {
  AD:"020",AE:"784",AF:"004",AG:"028",AL:"008",AM:"051",AO:"024",AR:"032",AT:"040",AU:"036",
  AZ:"031",BA:"070",BB:"052",BD:"050",BE:"056",BF:"854",BG:"100",BH:"048",BI:"108",BJ:"204",
  BN:"096",BO:"068",BR:"076",BS:"044",BT:"064",BW:"072",BY:"112",BZ:"084",CA:"124",CD:"180",
  CF:"140",CG:"178",CH:"756",CI:"384",CL:"152",CM:"120",CN:"156",CO:"170",CR:"188",CU:"192",
  CV:"132",CY:"196",CZ:"203",DE:"276",DJ:"262",DK:"208",DM:"212",DO:"214",DZ:"012",EC:"218",
  EE:"233",EG:"818",ER:"232",ES:"724",ET:"231",FI:"246",FJ:"242",FR:"250",GA:"266",GB:"826",
  GD:"308",GE:"268",GH:"288",GM:"270",GN:"324",GQ:"226",GR:"300",GT:"320",GW:"624",GY:"328",
  HN:"340",HR:"191",HT:"332",HU:"348",ID:"360",IE:"372",IL:"376",IN:"356",IQ:"368",IR:"364",
  IS:"352",IT:"380",JM:"388",JO:"400",JP:"392",KE:"404",KG:"417",KH:"116",KI:"296",KM:"174",
  KN:"659",KP:"408",KR:"410",KW:"414",KZ:"398",LA:"418",LB:"422",LC:"662",LI:"438",LK:"144",
  LR:"430",LS:"426",LT:"440",LU:"442",LV:"428",LY:"434",MA:"504",MC:"492",MD:"498",ME:"499",
  MG:"450",MH:"584",MK:"807",ML:"466",MM:"104",MN:"496",MR:"478",MT:"470",MU:"480",MV:"462",
  MW:"454",MX:"484",MY:"458",MZ:"508",NA:"516",NE:"562",NG:"566",NI:"558",NL:"528",NO:"578",
  NP:"524",NR:"520",NZ:"554",OM:"512",PA:"591",PE:"604",PG:"598",PH:"608",PK:"586",PL:"616",
  PT:"620",PW:"585",PY:"600",QA:"634",RO:"642",RS:"688",RU:"643",RW:"646",SA:"682",SB:"090",
  SC:"690",SD:"729",SE:"752",SG:"702",SI:"705",SK:"703",SL:"694",SM:"674",SN:"686",SO:"706",
  SR:"740",SS:"728",ST:"678",SV:"222",SY:"760",SZ:"748",TD:"148",TG:"768",TH:"764",TJ:"762",
  TL:"626",TM:"795",TN:"788",TO:"776",TR:"792",TT:"780",TV:"798",TZ:"834",UA:"804",UG:"800",
  US:"840",UY:"858",UZ:"860",VA:"336",VC:"670",VE:"862",VN:"704",VU:"548",WS:"882",YE:"887",
  ZA:"710",ZM:"894",ZW:"716",
};

const NUMERIC_TO_ALPHA2: Record<string, string> = {};
for (const [a2, num] of Object.entries(ALPHA2_TO_NUMERIC)) {
  NUMERIC_TO_ALPHA2[num] = a2;
}

interface Props {
  countryCounts: Record<string, number>;
}

export function WorldMap({ countryCounts }: Props) {
  const counts = Object.values(countryCounts);
  const maxCount = counts.length > 0 ? Math.max(...counts) : 1;

  function fillForGeo(geoId: string): string {
    const alpha2 = NUMERIC_TO_ALPHA2[geoId];
    if (!alpha2) return "#1c2128";
    const count = countryCounts[alpha2] ?? 0;
    if (count === 0) return "#21262d";
    const t = count / maxCount;
    const r = Math.round(30 + t * (88 - 30));
    const g = Math.round(80 + t * (166 - 80));
    const b = Math.round(180 + t * (247 - 180));
    return `rgb(${r},${g},${b})`;
  }

  return (
    <div className="mt-2">
      <p className="text-[11px] text-[#7d8590] mb-1">Respondents by country</p>
      <div className="rounded-[6px] overflow-hidden bg-[#0d1117] w-full" style={{ height: 160 }}>
        <ComposableMap
          width={800}
          height={400}
          projectionConfig={{ scale: 120, center: [0, 10] }}
          style={{ width: "100%", height: "100%" }}
        >
          <Geographies geography={topoData as object}>
            {({ geographies }: { geographies: Array<{ rsmKey: string; id: string }> }) =>
              geographies.map((geo) => (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
                  fill={fillForGeo(String(geo.id))}
                  stroke="#161b22"
                  strokeWidth={0.5}
                  style={{
                    default: { outline: "none" },
                    hover: { outline: "none" },
                    pressed: { outline: "none" },
                  }}
                />
              ))
            }
          </Geographies>
        </ComposableMap>
      </div>
    </div>
  );
}
